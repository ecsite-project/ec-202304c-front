...
import { BACK_IP, IMG_IP } from "../config";
...
const ItemDetail: React.FC = () => {
    ...
    const getItemAsync = async () => {
      ...
      if (token) {
        response = await axios.get(
          `http://${BACK_IP}:8080/ec-202404c/item/${id}`,
          {
            headers: {
  ...
  const onSubmit = async (data: ItemFormData) => {
    ...
    try {
      const response = await axios.post(
        `http://${BACK_IP}:8080/ec-202404c/cart/add`,
        cartItem
      );
  ...
  const handleFavoriteClick = async () => {
    ...
    try {
      if (isFavorite) {
        await axios.post(`http://${BACK_IP}:8080/ec-202404c/favorites/delete`, {
          userId: userInfo.userid,
          itemId: item.id,
        });
        setshowFavoriteToastDelete(true);
      } else {
        await axios.post(`http://${BACK_IP}:8080/ec-202404c/favorites/add`, {
          userId: userInfo.userid,
          itemId: item.id,
        });
                ...
                {item.itemType === "set" && (
                  <div className="order-last flex gap-4 lg:order-none lg:flex-col">
                    {item.topId > 0 && (
                      ...
                      >
                        <img
                          src={`http://${IMG_IP}:9090/img/${item.topImagePath}`}
                          loading="lazy"
                    ...
                    {item.bottomId > 0 && (
                      ...
                      >
                        <img
                          src={`http://${IMG_IP}:9090/img/${item.bottomImagePath}`}
                          loading="lazy"
                          alt="Photo by Himanshu Dewangan"
                          className="h-full w-full object-cover object-center transition duration-200 hover:opacity-75 transform transition-transform duration-300 ease-in-out hover:scale-110"
                        />
